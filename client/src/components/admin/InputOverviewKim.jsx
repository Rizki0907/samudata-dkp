...
import { Plus, Trash2, Edit2, Loader2, Save, X, AlertCircle, CheckCircle2, Database, Utensils } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
// eslint-disable-next-line no-unused-vars
import { cn } from '@/lib/utils';

export default function InputOverviewKim({ showToast, onDataChange }) {
  ...
  async function fetchOverviewKim() {
    try {
      setLoading(true);
      const res = await api.get('/master-data/OVERVIEW_KIM');
      if (res.data?.success) {
        setItems(res.data.data);
      }
    } catch (err) {
      console.error('Failed to load OVERVIEW_KIM', err);
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleOpenEdit = (item) => {
    ...
    setFormData({
      tahun: item.value || '',
      total_konsumsi: item.metadata?.total_konsumsi ?? ''
    });
    setIsModalOpen(true);
  };

  const handleDelete = async (id) => {
    if (!window.confirm('Apakah Anda yakin ingin menghapus data overview Konsumsi Ikan Masyarakat ini?')) return;
    try {
      const res = await api.delete(`/master-data/${id}`);
      if (res.data?.success) {
        showToast && showToast('Data overview KIM berhasil dihapus');
        fetchOverviewKim();
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      category: 'OVERVIEW_KIM',
      value: String(formData.tahun),
      metadata: {
        tahun: Number(formData.tahun),
        total_konsumsi: formData.total_konsumsi !== '' ? Number(formData.total_konsumsi) : ''
      }
    };
    ...
    try {
      ...
      if (isEditing) {
        ...
        if (res.data?.success) {
          showToast && showToast('Data overview KIM berhasil diperbarui');
        }
      } else {
        const res = await api.post('/master-data', payload);
        if (res.data?.success) {
          showToast && showToast('Data overview KIM berhasil ditambahkan');
        }
      }
      ...
      showToast && showToast(err.response?.data?.message || 'Gagal menyimpan data overview KIM', 'error');
    } finally {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-card border border-border p-6 rounded-2xl shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-foreground">Input Overview Konsumsi Ikan Masyarakat (KIM)</h2>
          </div>
        </div>
        <button
        ...
        {loading ? (
          ...
          <div className="text-center py-16 px-4">
            ...
            <p className="text-sm text-muted-foreground max-w-sm mx-auto">
              Silakan klik tombol "Tambah Overview" untuk memasukkan data ringkasan Konsumsi Ikan Masyarakat (KIM).
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-border bg-muted/40 text-xs font-bold uppercase tracking-wider text-muted-foreground">
                  ...
                  <th className="py-4 px-6 text-center">Total Konsumsi Ikan (Ton)</th>
                ...
                {items.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-muted/20 transition-colors">
                      ...
                      <td className="py-4 px-6 text-center text-foreground font-medium">
                        {meta.total_konsumsi !== undefined && meta.total_konsumsi !== '' && meta.total_konsumsi !== null
                          ? `${Number(meta.total_konsumsi).toLocaleString('id-ID', { maximumFractionDigits: 2 })} Ton`
                          : '-'}
                      </td>
            ...
            >
              <div className="flex items-center justify-between px-6 py-5 border-b border-border">
                <h3 className="text-lg font-bold text-foreground">
                  {isEditing ? 'Edit Overview Konsumsi Ikan Masyarakat' : 'Tambah Overview Konsumsi Ikan Masyarakat'}
                </h3>
                <button
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-5">
                ...
                {/* 2. Total Konsumsi Ikan (Ton) */}
                <div className="space-y-2">
                  <label className="text-sm font-medium text-foreground">
                    Total Konsumsi Ikan (Ton) <span className="text-rose-500">*</span>
                  </label>
                  <input
                    type="number"
                    step="any"
                    value={formData.total_konsumsi}
                    onChange={(e) => setFormData({ ...formData, total_konsumsi: e.target.value })}
