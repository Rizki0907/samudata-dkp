...
const kondisiLamunStyle = (kondisi) => {
  const k = kondisi || '';
  if (k.startsWith('Kaya')) return 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20';
  if (k.startsWith('Kurang Kaya')) return 'bg-amber-500/10 text-amber-400 border-amber-500/20';
      ...
      <div className="flex items-center gap-3 pb-4 border-b border-border">
        ...
        <div>
          <h2 className="text-lg font-bold text-foreground">
            {initialData ? 'Edit Data Padang Lamun' : 'Tambah Data Padang Lamun'}
          </h2>
          <p className="text-sm text-muted-foreground">Isi data kondisi dan rehabilitasi lamun sesuai standar DKP.</p>
        </div>
      </div>
