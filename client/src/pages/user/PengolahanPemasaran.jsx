...
import { Loader2, TrendingUp, MapPin, Factory, FileText, Box, LineChart, Users } from 'lucide-react';
...
/**
 * Kontrak data yang diharapkan dari backend:
 *
 * GET /pengolahan-pemasaran?tahun=&bulan=          -> { success, data: [...] } (hanya status APPROVED)
 * GET /pengolahan-pemasaran/stats?tahun=&bulan=     -> { success, stats: {
 *      produksiPerKabupaten: [{ name, produksi, nilai }],
 *      trenBulanan: [{ bulan: 'Januari', [jenisProduk]: number, ... , Lainnya: number }, ...],
 *      top5Jenis: ['Ikan Asap', 'Terasi', ...],
 *      komposisiKegiatan: [{ name, value }],   // treemap: jenis_kegiatan_pengolahan / jenis_kegiatan_pemasaran
 *      heatmapData: [{ kabupaten, bulan, produksi, normalized }],
...
export default function PengolahanPemasaran() {
  ...
  const getJenisDetail = row =>
    row.jenis_kegiatan === 'Pengolahan' ? row.jenis_kegiatan_pengolahan : row.jenis_kegiatan_pemasaran;

  const kabupatenOptions = useMemo(() => [...new Set(data.map(d => d.kabupaten_kota))].filter(Boolean).sort(), [data]);
  const tahunOptions = useMemo(() => [...new Set(data.map(d => String(d.tahun)))].filter(Boolean).sort((a, b) => Number(b) - Number(a)), [data]);
  ...
  const columns = useMemo(() => [
    ...
    { header: 'Nama UPI', accessorKey: 'nama_upi' },
    {
      ...
      cell: info => (<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-primary/10 text-primary">{info.getValue()}</span>)
    },
    { header: 'Jenis Detail', id: 'jenis_detail', cell: info => getJenisDetail(info.row.original) || '-' },
    { header: 'Skala Usaha', accessorKey: 'skala_usaha', cell: info => (<span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800">{info.getValue()}</span>) },
    { header: 'Jenis Produk', accessorKey: 'jenis_produk' },
    { header: 'Hasil Produksi/Tahun (Kg)', accessorKey: 'hasil_produksi_per_tahun_kg', cell: info => (info.getValue() || 0).toLocaleString('id-ID') },
    { header: 'Nilai Hasil/Tahun (Rp)', accessorKey: 'nilai_hasil_produksi_per_tahun_rp', cell: info => { const val = info.getValue() || 0; return new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(val); } },
    { header: 'Total Tenaga Kerja', accessorKey: 'total_seluruh_tenaga_kerja', cell: info => (info.getValue() || 0).toLocaleString('id-ID') }
  ], []);
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-heading font-bold text-foreground">Statistik Pengolahan & Pemasaran</h1>
        </div>
          ...
          <div className="bg-card border border-border rounded-2xl shadow-sm p-6">
            <div className="mb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h3 className="text-lg font-semibold text-foreground">Rincian Data Pengolahan & Pemasaran</h3>
                </div>
            ...
            <div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                ...
                <select value={filterJenisKegiatan} onChange={(e) => setFilterJenisKegiatan(e.target.value)} className="px-3 py-2 rounded-lg border border-border bg-card text-sm">
                  <option value="">Semua Jenis Kegiatan</option>
                  <option value="Pengolahan">Pengolahan</option>
                  <option value="Pemasaran">Pemasaran</option>
                </select>
                <select value={filterSkalaUsaha} onChange={(e) => setFilterSkalaUsaha(e.target.value)} className="px-3 py-2 rounded-lg border border-border bg-card text-sm">
              ...
              <DataTable
                ...
                formatExportData={(exportData) => exportData.map(row => ({
                  ...
                  'Hasil Produksi/Tahun (Kg)': row.hasil_produksi_per_tahun_kg,
                  'Nilai Hasil/Tahun (Rp)': row.nilai_hasil_produksi_per_tahun_rp,
                  'Total Tenaga Kerja': row.total_seluruh_tenaga_kerja
                }))}
              />
            </div>
          </div>
